...
def add_post(request,author_id):
    ...
    notification_message = f"{profile.user.username} has added a new post."
    
    # كل المستخدمين عدا صاحب المنشور لارسال الاشعار لهم
    all_users = Profile.objects.exclude(user_id=author_id)  

    #لكل مستخدم رح ابعتلو اشعار
    for user in all_users:
        ...
        notification_data_json = json.dumps(notification_data).encode('utf-8')
        ...
        try:
            with urlrequest.urlopen(req) as response:
                if response.status == 201:
                    print(f"Notification sent successfully to {user.user.username}.")
                else:
                    print(f"Failed to send notification to {user.user.username}.")
...
def add_comment(request,post_id,user_id):
    ...
    try:
        with urlrequest.urlopen(req) as response:
            if response.status != 201:
                print("Notification sent, but failed to confirm success.")
    except Exception as e:
        print(f"Error sending notification: {str(e)}")
