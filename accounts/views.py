...
def send_join_request(request, user_id, coach_id):
    try:
        ...
        notification_message = f"{trainer_profile.user.username} sent you a join request."
        notification_url = f"https://render-project1-qyk2.onrender.com/notification/send-notifications/{coach_profile.user.id}/"
        
        notification_data = json.dumps({
            'content': notification_message,
            'room_name': f'join_request_{coach_profile.user.id}',
        }).encode('utf-8')
        ...
        try:
            with urlrequest.urlopen(req) as response:
                if response.status == 201:
                    ...
                    return Response({"message": f"Join request sent successfully and notification sent. {notification_data}"}, status=201)
                else:
                    return Response({"message": "Join request sent, but failed to send notification."}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def respond_to_join_request(request, coach_id, request_id):
    try:
        ...
        print(join_request.trainer.user.id)
        ...
        if action == 'Accept':
            ...
            join_request.save()
            message_text = f"{coach_profile.user.username} accepted your join request"
    #        return Response({"message": "Join request accepted"}, status=200)
        # حالة الرفض 
        elif action == 'Reject':
            join_request.status = 'Rejected'
            join_request.save()
            message_text = f"{coach_profile.user.username} rejected your join request"
    #        return Response({"message": "Join request rejected"}, status=200)
        notification_url = f"https://render-project1-qyk2.onrender.com/notification/send-notifications/{join_request.trainer.user.id}/"
        notification_data = json.dumps({
            'content': message_text,
            'room_name': f'join_response_{join_request.trainer.user.id}',
        }).encode('utf-8')
        ...
        try:
            with urlrequest.urlopen(req) as response:
                if response.status == 201:
                    return Response({"message": f"Join request {action.lower()}ed and notification sent. {notification_data}"}, status=200)
                else:
                    return Response({"message": f"Join request {action.lower()}ed, but notification failed."}, status=500)
        ## حالات الفشل
        except HTTPError as e:
            return Response({"message": f"HTTP error: {e.code} - {e.reason}"}, status=500)
        except URLError as e:
            return Response({"message": f"Connection error: {e.reason}"}, status=500)
        except Exception as e:
